...
deeplabv3_Detector::deeplabv3_Detector(const std::string& _engine_file):
...
void deeplabv3_Detector::init_context()
{
...
void deeplabv3_Detector::destroy_context()
{
    bool cudart_ok = true;
...
deeplabv3_Detector::~deeplabv3_Detector()
{
    destroy_context();
    std::cout<<"Context destroyed for ["<<input_h<<"x"<<input_w<<"]"<<std::endl;
}

void deeplabv3_Detector::pre_process(cv::Mat& img)
{
...
void deeplabv3_Detector::do_detection(cv::Mat& img)
{
    assert(context != nullptr);
...
void deeplabv3_Detector::post_process(cv::Mat& img)
{
