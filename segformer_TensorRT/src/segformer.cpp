...
segformer_Detector::segformer_Detector(const std::string& _engine_file):
...
void segformer_Detector::init_context()
{
...
void segformer_Detector::destroy_context()
{
    bool cudart_ok = true;
...
segformer_Detector::~segformer_Detector()
{
    destroy_context();
    std::cout<<"Context destroyed for ["<<input_h<<"x"<<input_w<<"]"<<std::endl;
}

void segformer_Detector::pre_process(cv::Mat& img)
{
...
void segformer_Detector::do_detection(cv::Mat& img)
{
    assert(context != nullptr);
...
void segformer_Detector::post_process(cv::Mat& img)
{
    ...
    cv::warpAffine(output_index, output_index, m2x3_d2i, img.size(), cv::INTER_NEAREST);
